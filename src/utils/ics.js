...
export function buildIcsContent(spot, proposedTimeIso) {
  ...
  return [
    ...
    `DESCRIPTION:${escapeIcsText(`Picked together on Nomnom: ${spot.name}`)}`,
 ...
 */
export async function shareIcsForSpot(spot, proposedTimeIso) {
  const available = await Sharing.isAvailableAsync();
  if (!available) {
    throw new Error('SHARING_UNAVAILABLE');
  }
  const content = buildIcsContent(spot, proposedTimeIso);
